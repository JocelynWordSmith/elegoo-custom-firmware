...
int activeStreams = 0;
static const int MAX_STREAMS = 2;  // limited by DRAM (no PSRAM)
...
esp_err_t streamHandler(httpd_req_t *req)
{
  ...
  while (true)
  {
    ...
    // esp_camera_fb_get() blocks until next frame, providing a natural RTOS yield
  }
...
esp_err_t controlHandler(httpd_req_t *req)
{
  ...
  {
    if (value >= 0 && value <= 8)  // max VGA without PSRAM
    {
      res = s->set_framesize(s, (framesize_t)value);
    }
    else
    {
      httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "max framesize is 8 (VGA) without PSRAM");
      return ESP_FAIL;
    }
  }
